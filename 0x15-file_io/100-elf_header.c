 ...
 * If it's not an ELF file, exits with code 98.
 */
void check_elf(unsigned char *e_ident)
{
    int index;
...
/**
 * Prints the magic numbers of an ELF header, separated by spaces.
 */
void print_magic(unsigned char *e_ident)
{
    int index;
...
/**
 * Prints the operating system and ABI of an ELF header.
 */
void print_osabi(unsigned char *e_ident)
{
    printf(" OS/ABI: ");
...
/**
 * Closes an ELF file.
 * If the file cannot be closed, exits with code 98.
 */
void close_elf(int elf)
{
    if (close(elf) == -1)
 ...
 * If the file is not an ELF File or the function fails, exits with code 98.
 */
